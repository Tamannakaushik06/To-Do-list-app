...
class TodoListApp(tk.Tk):
    def __init__(self):
        ...
        self.configure(bg="#E0FFFF")  # Change background color here
        style = Style(theme="flatly")

        # Create title label
        ttk.Label(self, text="Todo List", font=("Arial", 28, "bold"), background="#d3d3d3").pack(pady=10)

        # Create input field for adding tasks
        self.task_input = ttk.Entry(self, font=("Arial", 14), width=30)
        ...
        ttk.Button(self, text="Add", command=self.add_task, style="success.TButton").pack(pady=5)

        # Create listbox to display added tasks
        self.task_list = tk.Listbox(self, font=("Arial", 14), height=10, selectmode=tk.NONE)
        self.task_list.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        # Create buttons for marking tasks as done or deleting them
        ttk.Button(self, text="Mark Done", style="primary.TButton", command=self.mark_done).pack(side=tk.LEFT, padx=10, pady=5)
        ttk.Button(self, text="Delete", style="danger.TButton", command=self.delete_task).pack(side=tk.RIGHT, padx=10, pady=5)
